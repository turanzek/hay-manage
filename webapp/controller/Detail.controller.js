sap.ui.define([
...
], function (JSONModel, BaseController, ColumnListItem, Label, Token, Fragment, Filter,
	FilterOperator, MessageBox, Dialog, Button, library, TextArea, uid, BusyIndicator,MessageToast,DateFormat,jQuer) {
	"use strict";
	...
	/* 		[oExitButton, oEnterButton].forEach(function (oButton) {
			...
			this.columnModel = new JSONModel(sap.ui.require.toUrl("gubretas/mm/hay/json") + "/columnsModel.json");
		...
		handleItemPress: function (oEvent) {
			...
			this.oRouter.navTo("detailDetail", {
				...
				product: this._itemNo, supplier: supplier
			});
		},
		handleFullScreen: function () {
			this.bFocusFullScreenButton = true;
			var sNextLayout = this.oModel.getProperty("/actionButtonsInfo/midColumn/fullScreen");
			this.oRouter.navTo("detail", { layout: sNextLayout, itemNo: this._itemNo });
		},
		...
		_onMasterMatched: function (oEvent) {
			this._itemNo = oEvent.getParameter("arguments").itemNo || this._itemNo || "0";
		...
		onValueHelpRequested: function (event) {
			...
			Fragment.load({
				...
				var filters = [new Filter("Name", FilterOperator.EQ, valueHelpname)];

				var bindingObject = this.getView().getBindingContext("application").getObject();
				if (valueHelpname === "Mtart") {
					if (bindingObject.MaterialKind !== undefined && bindingObject.MaterialKind !== "") {
						filters.push(new Filter("Criteria1", FilterOperator.EQ, bindingObject.MaterialKind));
					}
				}

				if (valueHelpname === "Lgort") {
					if (bindingObject.Plants.results[0].Werks !== undefined && bindingObject.Plants.results[0].Werks !== "") {
						filters.push(new Filter("Criteria1", FilterOperator.EQ, bindingObject.Plants.results[0].Werks));
					}
				}

				if (valueHelpname === "ExpandLgort") {
					if (bindingObject.Plants.results[0].Werks !== undefined && bindingObject.Plants.results[0].ExpandWerks !== "") {
						filters.push(new Filter("Criteria1", FilterOperator.EQ, bindingObject.Matnr));
						filters.push(new Filter("Criteria2", FilterOperator.EQ, bindingObject.Plants.results[0].ExpandWerks));
					}
				}

				if (valueHelpname === "ExpandWerks") {
					if (bindingObject.Plants.results[0].Werks !== undefined && bindingObject.Plants.results[0].ExpandWerks !== "") {
						filters.push(new Filter("Criteria1", FilterOperator.EQ, bindingObject.Matnr));
					}
				}

				if (valueHelpname === "Bklas") {
					if (bindingObject.Mtart !== undefined && bindingObject.Mtart !== "") {
						filters.push(new Filter("Criteria1", FilterOperator.EQ, bindingObject.Mtart));
					}
				}


				if (valueHelpname === "Matkl") {
					if (bindingObject.MaterialKind !== undefined && bindingObject.MaterialKind !== "") {
						filters.push(new Filter("Criteria1", FilterOperator.EQ, bindingObject.MaterialKind));
					}
				}
				...
				this.getOwnerComponent().getModel("valueModel").read("/ValueHelpSet", parameters);
		...
		onPressAction: function (event) {
			...
			this._handleDialog(dialogText, commentActive, obligatory,
				function (dialog, event) {
					...
					this.getOwnerComponent().getModel("mainModel").create('/ProcessModelSet', process, {
		...
		_applyValidation: function () {
			...
			for (var i = 0; i < inputs.length; i++) {
				...
				if (item.getRequired() && item.getVisible() && item.getEditable()) {
					if (item.getValue() === "" || item.getValue() === undefined || parseFloat(item.getValue()) === 0) {
		...
		_handleDialog: function (text, commentActive, obligatory, successCallback) {
		...
		onBeforeUploadStarts: function (oEvent) {
			...
			var oCustomerHeaderToken = new sap.ui.core.Item({
				key: "x-csrf-token",
				text: this.getOwnerComponent().getModel("mainModel").getSecurityToken()
			});
			...
			var oCustomerHeaderSlug = new sap.ui.core.Item({
				key: "slug",
				text: encodeURIComponent(oItemToUpload.getFileName() + ";" + bindingObject.RequestNo + ";" + bindingObject.Matnr + ";" + this.FileGuid)
			});
		...
		onUploadCompleted: function (event) {
			...
			if (event.getSource().getIncompleteItems().length === this.completeItemCount) {
		...
		onPressAddLine: function (event) {
			var initialLine = {};

			var table = event.getSource().getParent().getParent();


			var application = this.getView().getBindingContext("application").getObject();

			var fullPath = this.getView().getBindingContext("application") + "/" + table.getBinding().getPath();
			var mainPathArray = fullPath.split("/").slice(0, 6);
			var mainPath = mainPathArray.join("/");




			var lines = this.getView().getModel("application").getProperty(mainPath);


			if (mainPath.includes("Equipments")) {
				initialLine = {
					Guid: application.Guid,
					RequestNo: application.RequestNo,
					Matnr: application.Matnr,
					Equnr: ""
				}
			}
			else {
			...
			this.getView().getModel("application").setProperty(mainPath, lines);
			this.getView().getModel("application").refresh(true);
